...
const DASH_TEXT_EN = {
  'داشبورد':'Dashboard','اینباندها':'Inbounds','دسته‌بندی‌ها':'Categories','گروه‌های ساب':'Subscription Groups','پلن‌های فروش':'Sales Plans','گزارش‌ها':'Reports','ادمین‌ها':'Admins','فعالیت‌ها':'Activity','پیام‌ها':'Messages','تنظیمات':'Settings',
  'وضعیت لحظه‌ای سرویس، کانفیگ‌ها و ربات فروش':'Live service, configuration and sales-bot status','مدیریت حرفه‌ای اینباندها، کلاینت‌ها و لینک‌های اشتراک':'Professional inbound, client and subscription management','پیش‌فرض‌های حجم، انقضا و محدودیت برای گروه‌های کانفیگ':'Traffic, expiry and limit presets','ترکیب چند کانفیگ در یک لینک اشتراک واحد':'Combine multiple configs into one subscription link','پلن‌هایی که در ربات فروش تلگرام نمایش داده می‌شوند':'Plans displayed in the Telegram sales bot','خلاصه‌ی عملکرد فروش و کانفیگ‌ها':'Sales and configuration performance','حساب‌های دسترسی جانبی به پنل (فقط مالک)':'Secondary panel access accounts (owner only)','۱۵۰ رویداد اخیر پنل':'Latest 150 panel events','مرکز خطاها، هشدارها و پیام‌های سیستم':'Errors, warnings and system messages','آدرس عمومی پنل، ربات فروش تلگرام و رمز عبور':'Panel URL, Telegram sales bot and account security',
  'کل اینباندها':'Total Inbounds','فعال':'Active','مشتریان فروشگاه':'Store Customers','فروش (Stars)':'Sales (Stars)','در حال اجرا':'Running','خاموش':'Stopped','وضعیت نامشخص':'Unknown status','ذخیره':'Save','شروع':'Start','توقف':'Stop','کپی شد ✓':'Copied ✓','حذف شد':'Deleted','بروزرسانی شد':'Updated','ساخته شد':'Created','تغییر امن رمز':'Change Password','رمز فعلی':'Current Password','رمز جدید':'New Password','تکرار رمز جدید':'Repeat New Password','نام کاربری پنل':'Panel Username','تغییر نام کاربری':'Change Username','امنیت حساب':'Account Security','آدرس عمومی پنل':'Panel Public URL','ذخیره آدرس':'Save URL','ذخیره شبکه':'Save Network','پیام خوش‌آمد':'Welcome Message','منوی مدیریت':'Admin Menu','پیام ساخت کانفیگ':'Config Created Message','پیام فروشگاه':'Store Message','پیام پرداخت موفق':'Payment Success Message','ذخیره متن‌های ربات':'Save Bot Texts','مرکز کنترل':'Control Center','بروزرسانی زنده':'Live Refresh','بارگذاری کامل رابط':'Hard Reload','دامنه پنل کپی شد ✓':'Panel URL copied ✓',
  'مدیریت کلاینت‌های واقعی':'Real Client Manager','ساخت کلاینت واقعی':'Create Client','حجم':'Traffic','انقضا':'Expiry','کپی VLESS':'Copy VLESS','کنترل':'Control','جزئیات':'Details','جستجو':'Search','فیلتر':'Filter','لغو':'Cancel','تأیید':'Confirm','بستن':'Close','ویرایش':'Edit','ساخت':'Create','فعال‌سازی':'Enable','غیرفعال‌سازی':'Disable','عملیات':'Actions','پروتکل':'Protocol','پورت':'Port','سرعت':'Speed','محدودیت IP':'IP Limit','حجم مصرفی':'Usage','اتصال‌ها':'Connections','خطا':'Error','هشدار':'Warning','اطلاعات':'Info'
};
const DASH_ORIGINAL_TEXT = new WeakMap();
const DASH_ORIGINAL_ATTR = new WeakMap();
function dashTranslateValue(value, lang){
  if(lang==='fa') return value;
  let out=value; for(const [fa,en] of Object.entries(DASH_TEXT_EN)){ if(out.includes(fa)) out=out.split(fa).join(en); } return out;
}
function translateDashNode(root, lang){
  const walker=document.createTreeWalker(root,NodeFilter.SHOW_TEXT); let n;
  while(n=walker.nextNode()){ if(n.parentElement?.closest('script,style,textarea')) continue; if(!DASH_ORIGINAL_TEXT.has(n)) DASH_ORIGINAL_TEXT.set(n,n.nodeValue); const original=DASH_ORIGINAL_TEXT.get(n); const next=lang==='en'?dashTranslateValue(original,'en'):original; if(n.nodeValue!==next) n.nodeValue=next; }
  root.querySelectorAll('input[placeholder],button[title],input[title],textarea[placeholder]').forEach(el=>{
    ['placeholder','title'].forEach(a=>{ if(!el.hasAttribute(a)) return; if(!DASH_ORIGINAL_ATTR.has(el)) DASH_ORIGINAL_ATTR.set(el,{}); const store=DASH_ORIGINAL_ATTR.get(el); if(store[a]===undefined) store[a]=el.getAttribute(a)||''; const next=lang==='en'?dashTranslateValue(store[a],'en'):store[a]; if(el.getAttribute(a)!==next) el.setAttribute(a,next); });
  });
}
let DASH_TRANSLATING=false;
function applyDashLang(lang){
  const d = DASH_I18N[lang] || DASH_I18N.fa;
  document.querySelectorAll('#tabnav [data-i18n]').forEach(el=>{ const k=el.getAttribute('data-i18n'); if(d[k]!==undefined) el.textContent=d[k]; });
  document.getElementById('dashLangFa').classList.toggle('on', lang==='fa'); document.getElementById('dashLangEn').classList.toggle('on', lang==='en');
  document.documentElement.setAttribute('dir', d.dir); document.documentElement.setAttribute('lang', lang);
  document.body.classList.toggle('dash-en', lang==='en'); updatePageTitle(CURRENT_PAGE);
  translateDashNode(document.body,lang);
}
const DASH_TRANSLATOR = new MutationObserver(()=>{ if(!DASH_TRANSLATING && getDashLang()==='en'){ DASH_TRANSLATING=true; try{translateDashNode(document.body,'en');}finally{DASH_TRANSLATING=false;} } });
DASH_TRANSLATOR.observe(document.body,{subtree:true,childList:true});
function setDashLang(lang){
  try{ localStorage.setItem('vw_dash_lang', lang); }catch(e){}
  applyDashLang(lang);
}
const DASH_EN_TERMS = {
  'داشبورد':'Dashboard','اینباندها':'Inbounds','اینباند':'Inbound','دسته‌بندی‌ها':'Categories','دسته‌بندی':'Category','گروه‌های ساب':'Sub Groups','گروه ساب':'Sub Group','پلن‌های فروش':'Sale Plans','پلن فروش':'Sale Plan','گزارش‌ها':'Reports','ادمین‌ها':'Admins','ادمین':'Admin','مالک':'Owner','فعالیت‌ها':'Activity','فعالیت':'Activity','پیام‌ها':'Messages','تنظیمات':'Settings',
  'فعال':'Active','غیرفعال':'Inactive','خاموش':'Offline','در حال اجرا':'Running','روشن':'Online','آنلاین':'Online','آفلاین':'Offline','بروزرسانی':'Refresh','ذخیره':'Save','ذخیره تغییرات':'Save Changes','حذف':'Delete','ویرایش':'Edit','ساخت':'Create','لغو':'Cancel','تأیید':'Confirm','انصراف':'Cancel','بازگشت':'Back','نمایش':'View','کپی':'Copy','کپی لینک':'Copy Link','دریافت فایل':'Download','جستجو':'Search','فیلتر':'Filter','همه':'All','خطا':'Error','هشدار':'Warning','مرورگر':'Browser','سیستم':'System','وضعیت':'Status','زمان':'Time','نوع':'Type','پیام':'Message','نام کاربری':'Username','رمز عبور':'Password','رمز جدید':'New Password','رمز فعلی':'Current Password','تکرار رمز عبور':'Repeat Password','تغییر امن رمز':'Change Password','تغییر نام کاربری':'Change Username','لغو نشست‌های قبلی':'Revoke Previous Sessions',
  'آدرس عمومی پنل':'Panel Public URL','امنیت حساب':'Account Security','ربات':'Bot','مدیریت ربات':'Bot Management','کنترل پنل':'Panel Control','کنترل سنتر':'Control Center','مرکز پیام و خطا':'Message & Error Center','لاگ فعالیت‌ها':'Activity Log','مرکز خطاها':'Error Center','مدیریت ادمین':'Admin Management','مدیریت حرفه‌ای اینباندها، کلاینت‌ها و لینک‌های اشتراک':'Professional inbound, client and subscription management','وضعیت لحظه‌ای سرویس، کانفیگ‌ها و ربات فروش':'Live service, configuration and sales-bot status',
  'مشتریان':'Customers','مشتری‌ها':'Customers','کلاینت‌ها':'Clients','کلاینت':'Client','مصرف':'Usage','حجم':'Traffic','انقضا':'Expiry','روز':'Days','ساعت':'Hours','دقیقه':'Minutes','پورت':'Port','سرعت':'Speed','اتصال':'Connection','اتصالات':'Connections','پروتکل':'Protocol','شبکه':'Network','امنیت':'Security','نامحدود':'Unlimited','پیش‌فرض':'Default','اختیاری':'Optional','اجباری':'Required','توضیحات':'Description','نام':'Name','تعداد':'Count','کل':'Total','آخرین ورود':'Last Login','رویدادی وجود ندارد.':'No events found.','لاگی وجود ندارد':'No activity logs.','داده‌ای موجود نیست':'No data available.','سیستم سالم است.':'System is healthy.','خطای ثبت‌شده‌ای وجود ندارد. سیستم سالم است.':'No recorded errors. System is healthy.','همگام‌سازی خودکار':'Auto sync','آخرین بروزرسانی':'Last updated',
  'تنظیمات و استودیو ظاهر':'Settings & Appearance Studio','بازنشانی ظاهر':'Reset Appearance','Appearance Studio':'Appearance Studio','فونت رابط کاربری':'Interface Font','اندازه متن':'Text Size','تراکم پنل':'Panel Density','گوشه‌ها':'Corners','پوسته':'Theme','رنگ اصلی':'Accent Color','انیمیشن‌های پنل':'Panel Animations','سایدبار باز در دسکتاپ':'Open sidebar on desktop','نورپردازی':'Glow','ظاهر پنل به حالت پیش‌فرض برگشت ✓':'Appearance reset ✓','فونت پنل تغییر کرد ✓':'Panel font updated ✓',
  'آدرس TCP ذخیره شد':'TCP address saved','آدرس ذخیره شد':'Address saved','ربات روشن شد':'Bot started','ربات خاموش شد':'Bot stopped','ربات در حال اجراست':'Bot is running','ربات خاموش است':'Bot is stopped','ذخیره شد':'Saved','ذخیره شد ✓':'Saved ✓','ذخیره تغییرات':'Save changes','ساخته شد':'Created','حذف شد':'Deleted','بروزرسانی شد':'Updated','کپی شد ✓':'Copied ✓','کپی شد':'Copied','نام کاربری با موفقیت تغییر کرد ✓':'Username changed successfully ✓','رمز عبور با موفقیت تغییر کرد ✓':'Password changed successfully ✓','خطا در پردازش اطلاعات ورود.':'Login data could not be processed.'
};
function translateDashTree(lang){
  const root=document.getElementById('app'); if(!root) return;
  const terms=Object.keys(DASH_EN_TERMS).sort((a,b)=>b.length-a.length);
  const walker=document.createTreeWalker(root,NodeFilter.SHOW_TEXT);
  const nodes=[]; while(walker.nextNode()) nodes.push(walker.currentNode);
  for(const node of nodes){
    const el=node.parentElement; if(!el || /^(SCRIPT|STYLE|TEXTAREA|INPUT|SELECT|OPTION)$/i.test(el.tagName) || el.closest('.mono')) continue;
    if(lang==='fa'){
      if(node.parentElement.dataset.vwFa !== undefined) node.nodeValue=node.parentElement.dataset.vwFa;
      continue;
    }
    if(node.parentElement.dataset.vwFa===undefined) node.parentElement.dataset.vwFa=node.nodeValue;
    let v=node.nodeValue;
    for(const term of terms){ if(v.includes(term)) v=v.split(term).join(DASH_EN_TERMS[term]); }
    node.nodeValue=v;
  }
  document.querySelectorAll('input[placeholder],textarea[placeholder]').forEach(el=>{
    if(lang==='fa'){ if(el.dataset.vwFaPlaceholder!==undefined) el.placeholder=el.dataset.vwFaPlaceholder; return; }
    if(el.dataset.vwFaPlaceholder===undefined) el.dataset.vwFaPlaceholder=el.placeholder;
    for(const term of terms) if(el.placeholder.includes(term)) el.placeholder=el.placeholder.split(term).join(DASH_EN_TERMS[term]);
  });
}
let dashTranslateObserver=null;
function enableDashTranslation(){
  if(dashTranslateObserver) dashTranslateObserver.disconnect();
  dashTranslateObserver=new MutationObserver(()=>{ if(getDashLang()==='en') translateDashTree('en'); });
  dashTranslateObserver.observe(document.getElementById('app'),{subtree:true,childList:true,characterData:true});
}
