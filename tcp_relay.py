# tcp_relay.py
# سرور VLESS خام روی TCP — مستقل از relay_vless.py (که روی WebSocket کار می‌کنه)
#
# چرا فایل جدا و پورت جدا؟
# کل main.py روی FastAPI/uvicorn اجرا می‌شه که فقط HTTP/WebSocket سرو می‌کنه.
# VLESS-TCP «خام» نیاز به یک TCP listener مستقل (asyncio.start_server) داره که
# بایت‌های خام رو می‌خونه، نه یک درخواست HTTP. به همین خاطر این سرویس روی یک
# پورت جداگانه (پیش‌فرض 6543، قابل تغییر با TCP_LISTEN_PORT) بالا میاد.
#
# نکته‌ی مهم برای Railway: پلن پیش‌فرض Railway فقط یک پورت HTTP عمومی
# (همون PORT اصلی) رو expose می‌کنه. برای این‌که این پورت TCP از بیرون در
# دسترس باشه باید از قابلیت «TCP Proxy» ریلوی (در تنظیمات سرویس، تب Networking)
# استفاده کنی؛ ریلوی یک آدرس و پورت عمومی به تو می‌ده که باید همون رو در تب
# «تنظیمات پنل» بخش «آدرس/پورت عمومی TCP» دستی وارد کنی — دقیقاً همون کاری که
# خواسته بودی (خودت آدرس رو بنویسی).
#
# امنیت: این نسخه با security=none (بدون TLS) کار می‌کنه — یعنی ترافیک بین
# کلاینت و این سرور رمزنگاری TLS نداره (فقط لایه‌ی VLESS). اگه روی Railway از
# TCP Proxy استفاده کنی، ترافیک بین Railway و کاربر نهایی هم رمزنگاری‌شده نیست
# مگر این‌که خودت یک لایه‌ی TLS/stunnel جلوش بذاری. برای اکثر سناریوهای ریسلری
# این روش رایج و قابل قبوله، ولی صرفاً برای شفافیت گفته باشیم.
